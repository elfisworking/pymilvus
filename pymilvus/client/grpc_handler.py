...
class GrpcHandler:
    ...
    def search_with_expression(self, collection_name, data, anns_field, param, limit, round_decimal,
                               expression=None, partition_names=None,
                               output_fields=None, timeout=None, **kwargs):
        _kwargs = copy.deepcopy(kwargs)
        collection_schema = self.describe_collection(collection_name, timeout)
        auto_id = collection_schema["auto_id"]
        _kwargs["schema"] = collection_schema
        requests = Prepare.search_requests_with_expr(collection_name, data, anns_field, param, limit, round_decimal,
                                                     expression, partition_names, output_fields, **_kwargs)
        _kwargs.pop("schema")
